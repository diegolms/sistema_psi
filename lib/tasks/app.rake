...
namespace :app do
  ...
  task :vencimentos_vencidos => :environment do
    vencimento = Vencimento.where("status = #{Vencimento::ABERTO} and data = '#{Time.now.beginning_of_month}'").each do |lancamento|
      lancamento.status = Vencimento::VENCIDO
      p lancamento.save!
    end
  end
end
